...
(async () => {
  ...
  try {
    const targetId = '.sy_zhuanti_news';
    ...
    const academician = await page.evaluate(async () => {
      ...
      return Array.from(container.children[0].querySelectorAll('a')).map(el => ({
        href: el.href,
    ...
    await page.waitForSelector('.right_md_titltop', {
      timeout: 500,
    });

    const introduce = await page.evaluate(async () => {
      return document.querySelector('.right_md_titltop').innerHTML;
    });
