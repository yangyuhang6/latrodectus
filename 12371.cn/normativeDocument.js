...
(async () => {
  ...
  try {
    ...
    const targetId = '#SUBD1590135292255503';
    await page.goto('https://www.12371.cn/special/zcwj/');
    ...
    const normativeDocumentDetail = [];

    while (true) {
      const detail = await page.evaluate(async () => {
        const container = document.querySelector(await targetId());

        if (container === null) throw new Error(`${targetId} is not exists`);
      ...
      normativeDocumentDetail.push(...detail);
    ...
    print(normativeDocumentDetail, normativeDocumentDetail.length);
  } catch (e) {
    print(e.message);
  }
